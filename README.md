...
출석은 **성실도**만 잽니다. 실제 KPI는 **카톡방 유입**이라 따로 잡습니다.

- 파트너는 제출 시 **사용한 글감**을 고르고(`submissions.drop_id`),
  이틀 뒤 작업실에서 성과를 입력합니다.
  - `leads`(카톡방 유입) = **필수.** 이게 KPI이자 "성과 보고됨"(`perf_at`)의 기준.
  - `views` / `comments` = **선택.** 입력 부담을 줄여 입력률을 올리려는 것. 비면 NULL.
  - 그래서 평균 조회는 분모가 다릅니다 — `views`가 들어온 건수로만 나눕니다(`n_views`).
- 스레드 조회수 자동 수집은 보류. 게시물 페이지에 조회수가 공개돼 있지만 JS 렌더라
  표준 라이브러리로는 못 긁고(헤드리스 브라우저 필요), 약관 리스크도 있습니다.
  자동화가 필요해지면 공식 Threads API(OAuth, `threads_manage_insights`)가 정답입니다.
  단 **카톡방 유입은 어떤 API로도 못 얻습니다** — 이 칸은 영구히 수동입니다.
- 운영자 `/perf` 에서:
