...
def dashboard_card(token=None, admin=False):
    ...
    try:
        ...
        conn.close()
    href = '/op/videos' if admin else ('/videos?t=' + quote(token, safe='') if token else '/videos')
    ...
    label = "영상 확인·관리 →" if admin else "영상 확인하고 받기 →"
    body += f"<a class=lk href='{esc(href)}'>{label}</a>"
    return body + '</section>'
